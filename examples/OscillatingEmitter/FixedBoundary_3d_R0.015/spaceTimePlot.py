...
for line in fo:
  if "timeStep" in line and Set<Nsets:
    ...
    splitline = line.split()
    R = float(splitline[1])
    X[Set,:] = np.linspace(R, Xmax, num=Npoints)/lambdaa
    line = next(fo)
    line = next(fo)
    i = 0
...
#plt.yticks([0, 2, 4, 6, 8 , 10], ['$0$', '$2$', '$4$', '$6$', '$8$', '$10$'], fontsize=fontsize)
#cb.set_ticks([-1, 0, 1])
...
#plt.savefig("../../figures/rt_oscilattingEmitter_staticBoundary.pdf", bbox_inches = "tight", dpi=400)
#plt.close()
plt.show()
