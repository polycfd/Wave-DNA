...
{
  MovingBoundary->R = MovingBoundary->R0 + (RunOptions->WaveExcitation.MovingBoundaryVelocityAmplitude)*(time - RunOptions->WaveExcitation.BoundaryMotionStartTime);
...
int BoundaryMotionOscillating(struct DNA_RunOptions *RunOptions, struct DNA_MovingBoundary *MovingBoundary, struct DNA_FluidProperties *FluidProperties, DNA_FLOAT time)
{
  ...
  MovingBoundary->R = MovingBoundary->R0 + (RunOptions->WaveExcitation.MovingBoundaryVelocityAmplitude)/(Omega + DNA_SMALL)*DNA_SIN((time - RunOptions->WaveExcitation.BoundaryMotionStartTime)*Omega);
  ...
  MovingBoundary->Udot = -RunOptions->WaveExcitation.MovingBoundaryVelocityAmplitude*(Omega + DNA_SMALL)*DNA_SIN((time - RunOptions->WaveExcitation.BoundaryMotionStartTime)*Omega);
...
int BoundaryMotionStationaryBlackHole(struct DNA_RunOptions *RunOptions, struct DNA_MovingBoundary *MovingBoundary, struct DNA_FluidProperties *FluidProperties, DNA_FLOAT time)
{
  ...
  MovingBoundary->Udot = -k*DNA_POW2(FluidProperties->c0)*DNA_POW(RunOptions->radius_horizon,(DNA_FLOAT)2.0*k)*DNA_POW(arg, min_2kplusone_by_kplusone);
